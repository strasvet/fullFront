...
const app = express();
const mysqlPool = mysql.createPool({connectionLimit:10, host:"localhost",user: "root",password: "root", database:"albums"});


app.set("view engine", "ejs");

function sendAlbumsList(request, response) {
    mysqlPool.query("SELECT * FROM albums", (error, rows)=> error ? console.error(error) : response.send(rows));
    //listFiles("pic", (f, stats) => stats.isDirectory()).then(a => response.send(a));
}

function sendPhotosList(request, response) {
    //listFiles(`pic/${request.params.album}`, (f) => f => /\.jpe?g$/i.exec(f)).then(a => response.send(a), console.error);
    const f = r => r.photo_filename;
    mysqlPool.query("SELECT * FROM photos WHERE album_id=?",[request.params.album],(error,rows)=>response.send(rows.map(f)));
}
...
function sendFile(request, response) {
  ...
  //      response.sendFile(__dirname + "/pic/" + `${request.params.album}/${request.params.file}`), console.error);
    response.sendFile(`${__dirname}/pic/${request.params.file}`);
}
...
const storageConfig = {
    ...
    destination: (request, file, cb) => cb(null, `pic/`),
...
function fileFilterM(request, file, callback) {
    ...
    //mysqlPool.query("INSERT photos VALUES(?,?)",[request.body.album,`${file.originalname}`]),
    (file.mimetype === "image/jpeg") ? callback(null, true) : callback("JPEG file expected");
    //mysqlPool.query("INSERT photos VALUES(?,?)",[request.body.album,file.originalname]);
}
...
function photoAdded(request, response, error) {
    if (error) {
        ...
        mysqlPool.query("INSERT photos VALUES(?,?)",[request.body.album,`${request.file.originalname}`]);
        //console.log(request.body.album,request.file.originalname);
        request.flash("success", "Photo addres successfully");
